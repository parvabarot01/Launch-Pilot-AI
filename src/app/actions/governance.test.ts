...
test("decideApprovalCore: rejecting does not change the rollout percentage", async (t) => {
  ...
  assert.equal((await readDb()).approvals[0].status, "rejected");
  assert.ok((await readDb()).auditLog.some((e) => e.action === "approval.rejected"));
});
...
test("decideApprovalCore: a member cannot decide (admin/owner only)", async (t) => {
  ...
  assert.equal((await readDb()).approvals[0].status, "pending");
});
...
test("decideApprovalCore: cannot decide the same request twice", async (t) => {
  ...
  assert.equal((await readDb()).approvals[0].status, "approved", "the first decision must stick");
});
